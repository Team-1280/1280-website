...
const Contact = () => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    // Vite-specific environment variable extraction
    const SERVICE_ID = import.meta.env.VITE_EMAILJS_SERVICE_ID || '';
    const TEMPLATE_ID = import.meta.env.VITE_EMAILJS_TEMPLATE_ID || '';
    const PUBLIC_KEY = import.meta.env.VITE_EMAILJS_PUBLIC_KEY || '';

    // Safeguard check to log if keys are missing in your local environment
    if (!SERVICE_ID || !TEMPLATE_ID || !PUBLIC_KEY) {
      console.error("Missing EmailJS environment configuration variables. Check your .env file!");
    }
    ...
    try {
      await emailjs.send(
        ...
        TEMPLATE_ID,
        {
          name: `${formData.firstName} ${formData.lastName}`.trim(), // Matches {{name}}
  ...
  return (
    <div className="bg-[#151417] text-zinc-100 min-h-screen font-sans selection:bg-[#fcc307] selection:text-[#1f1e23]">
      ...
      {/* 1. Header Hero Area */}
      <section className="bg-[#1f1e23] border-b border-zinc-800/80 py-24 px-4 md:px-12 text-center relative overflow-hidden">
        <img 
          src="/ContactUs1.jpg" 
